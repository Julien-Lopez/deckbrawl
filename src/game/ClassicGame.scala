...
class ClassicGame(override val interface: GameInterface) extends Game(interface) {
  override val startHand: Int = 5
  override val startLife: Int = 30
  val nbCardsDrawnInDrawPhase: Int = 1

  override def start(teams: Array[Team]): Unit = {
